...
color specularColor(const color& mat, const color& lightColor,
	...
	const dvec3& r, const dvec3& v) {
	if (dot(r, v) >= 0) {
		return lightColor * mat * pow(dot(r, v), shininess);
	}
	return dvec3(0, 0, 0);
...
color totalColor(const Material& mat, const color& lightColor,
	...
	dvec3 l = normalize(lightPos - intersectionPt );
	dvec3 r = normalize(2 * dot(l, n) * n - l);
	double d = length(lightPos - intersectionPt);
	if (attenuationOn){
		double AT = 1.0 / (ATparams.constant + ATparams.linear * d + ATparams.quadratic * pow(d, 2.0));
		return ambientColor(mat.ambient, lightColor)
			+ AT * (diffuseColor(mat.diffuse, lightColor, l, n)
				+ specularColor(mat.specular, lightColor, mat.shininess, r, v));
	} 
	else {
		return ambientColor(mat.ambient, lightColor)
...
color PositionalLight::illuminate(const dvec3& interceptWorldCoords,
	...
	if (!isOn) {
		...
		return totalColor(material, lightColor,
			normalize(eyeFrame.origin - interceptWorldCoords), normal, pos, 
