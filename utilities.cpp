...
double areaOfTriangle(double x1, double y1, double x2, double y2, double x3, double y3) {
	double sideA = distanceBetween(x1, y1, x2, y2);
	double sideB = distanceBetween(x2, y2, x3, y3);
	double sideC = distanceBetween(x1, y1, x3, y3);
...
double directionInRadians(const dvec2& referencePt, const dvec2& targetPt) {
	...
	double result = glm::atan(y, x);
	if (result < 0) {
		return 2 * glm::pi<double>() + result;
	}
	else {
		return result;
...
double directionInRadians(const dvec2& targetPt) {
	/* CSE 386 - todo  */
	double result = glm::atan(targetPt[1],targetPt[0]);
	if (result < 0) {
		return 2 * glm::pi<double>() + result;
	}
	else {
...
vector<double> quadratic(double A, double B, double C) {
	...
	if (discriminant > 0) {
		double ans1 = (-B + glm::sqrt(discriminant)) / (2 * A);
		double ans2 = (-B - glm::sqrt(discriminant)) / (2 * A);
		if (ans1 > ans2)
		{
...
double areaOfTriangle(const dvec3& pt1, const dvec3& pt2, const dvec3& pt3) {
	return 1.0/2.0 * areaOfParallelogram(pt2-pt1,pt3-pt1);
}
...
dvec3 pointingVector(const dvec3& pt1, const dvec3& pt2) {
	dvec3 answer = pt2 - pt1;
	if ( glm::length(answer) > 1){
